...
compute_mfcc() {
    ...
    for filename in $(sort $*); do
        mkdir -p $(dirname $w/mfcc/$filename.mfcc)
        EXEC="wav2mfcc 16 $db/$filename.wav $w/mfcc/$filename.mfcc"
...
compute_lpcc() {
    ...
    for filename in $(sort $*); do
		mkdir -p $(dirname $w/$FEAT/$filename.$FEAT)
        EXEC="wav2lpcc 16 $db/$filename.wav $w/$FEAT/$filename.$FEAT"
        echo $EXEC && $EXEC || exit 1
    done
}
...
for cmd in $*; do
   ...
   if [[ $cmd == train ]]; then
       ...
       for dir in $db_devel/BLOCK*/SES* ; do
           name=${dir/*\/}
           echo $name ----
           EXEC="gmm_train -v 1 -T 0.0001 -N 50 -m 64 -d $w/$FEAT -e $FEAT -i 1 -g $w/gmm/$FEAT/$name.gmm $lists/class/$name.train"
           echo $EXEC && $EXEC || exit 1
           echo
       done
   elif [[ $cmd == test ]]; then
       ...
       EXEC="gmm_train -v 1 -i 1 -T 1.e-6 -N 60 -m 100 -d $w/$FEAT -e $FEAT -g $w/gmm/$FEAT/$world.gmm $lists/verif/$world.train"
       echo $EXEC && $EXEC || exit 1
